...
function Review({ user, setUser }) {
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (response.ok) {
      const newBookReviews = bookReviews.filter(
        (bookReview) => bookReview.id !== id
      );
    ...
    <div className="reviews">
      <div className="book-details-content book_details">
        ...
        <div className="book-details-info">
          ...
          <div className="book-details-item">
            {bookReviews?.map((returnedReview) => (
              <ReviewCard
                key={returnedReview.id}
                returnedReview={returnedReview}
                user={user}
      ...
      <div className="review_form">
        <form className="max-w-[400px] w-full mx-auto rounded-lg bg-purple-500 p-8 px-8">
          <input
            type="text"
            className="w-full p-2 rounded-lg bg-white mt-2 focus:border-blue-500 focus:bg-white focus:outline-none"
            id="comment"
            name="name"
      ...
      <BookDiscussions book_id={book_id} user={user} />
    </div>
  );
}
